##   PETA CLUSTER K-MEANS JAWA BARAT (DATA BPS)
# 0. LOAD LIBRARY
library(readxl)
library(dplyr)
library(sf)
...
cluster_kmeans <- read_excel(
  "D:/Pascasarjana IPB/Semester 1/Pemograman Statistika/Pempro Project/Clustering K-Means.xlsx")
...
peta_jabar <- indo_jabar %>%
  left_join(cluster_kmeans, by = "KabKota_join")
...
ggplot(peta_jabar_single) +
  ...
  labs(
    title = "Peta Cluster Kemiskinan Jawa Barat (K-Means)"
  ) +
